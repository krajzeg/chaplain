...
module.exports = {
  ...
  create(config, test) {
    ...
    const jsonOptions = _.extend({}, config.json, test.json);

    return {
      compare(actual, expected) {
        const [actualJson, expectedJson] = [actual, expected].map(str => {
          let json = JSON.parse(str);
          json = removeIgnoredProperties(json, jsonOptions.ignore);
          return json;
        });
...
function removeIgnoredProperties(content, ignoredProps = []) {
  _.each(ignoredProps, prop => {
    _.set(content, prop, undefined);
  });
  return content;
}
