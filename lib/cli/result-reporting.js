...
const STATUS_MESSAGES = {
  ...
  'mime type changed':
    (t) => ({added: `${t} has changed its mime type`}),
...
export default function reportTestResult(testName, result, output) {
  ...
  switch(result.status) {
    ...
    case 'mime type changed': reportNewType(result, output); break;
...
function reportNew(result, output) {
  const relevantProps = _.omit(result.actual.props, ['type']);
...
function reportNewType(result, output) {
  const relevantProps = _.omit(result.actual.props, ['type']);
  const propsMsg = _.map(relevantProps, (value, key) => `${key}: ${value}`).join(", ");

  output.log("Your test started returning ", {added: result.actual.props.type},
    " instead of ", {removed: result.blessed.props.type});
  output.log("The test now returns ", {highlighted: propsMsg});
  output.log("and the new ", {highlighted: "contents"}, " are:\n");
  output.log(result.actual.value);
}
