...
function readSuite(cfg) {
  ...
  return Promise.resolve().then(() => {
    try {
      const modulePath = path.join(process.cwd(), cfg.suiteFile).replace(/\\/g, '/');
      return require(modulePath)();
    } catch(err) {
      if (err.code == 'MODULE_NOT_FOUND') {
        throw new UserFacingError(["Your ", {highlighted: cfg.suiteFile}, " test file seems to be missing."]);
      } else {
        throw new UserFacingError([
          "Your ", {highlighted: cfg.suiteFile}, ` test file failed to parse: ${err.message}`
        ]);
      }
    }
  });
