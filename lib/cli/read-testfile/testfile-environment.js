...
export default function makeTestEnvironment(testFilePath) {
  ...
  function createSandbox() {
    ...
    let globals = {
      ...
      process, console, // give all the node globals to the sandbox
...
export class SuiteContext {
  ...
  test(name, objOrFn, fn) {
    ...
    if (this.suite.tests.filter(t => t.name == name).length) {
      throw new Error(`Suite '${this.suite.name}' already has a test called '${name}'.`);
    }
