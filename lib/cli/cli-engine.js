...
import readTestfile from './read-testfile/read-testfile';
...
import {disableConsoleLog, enableConsoleLog} from './console-log';
...
export default function cliEngine({argv, fs, write, writeError, prompt, isTTY}) {
  ...
  function validateConfig(config, command) {
    ...
    // bless is only allowed with explicit tests
    if (config.command == 'bless' && !config.onlyTestsMatching) {
      output.error({sad: `You have to specify which tests to bless.\n`});
      outputUsage(output);
      throw new TerminationError(2, "No tests specified.");
    }
  ...
  function runAll(suites, cfg) {
    ...
    const runAllSuitesAndMergeResults = Promise.resolve()
      .then(() => disableConsoleLog())
      .then(() => {
    ...
    return runAllSuitesAndMergeResults
      ...
      .catch(err => {
        output.log(''); // in case of error, we want to start output later from a clean line
        throw err;
      });
    ...
    function reportResults({success, results}) {
      ...
      if (success) {
        ...
        const reports = _(results).toPairs().filter(([testKey, result]) => result.status != 'ok').value();
        const blessings = {};
        const reportEverything = _.reduce(reports, (promise, [key, result]) => {
          return promise
            .then(() => report(result))
            .then(blessed => blessings[key] = blessed);
        }, Promise.resolve());
        ...
        return reportEverything.then(() => {
          if (_.every(blessings, blessed => blessed)) {
            ...
            const unblessedResults = _.filter(results, (result, key) => {
              return (result.status != 'ok') && (!blessings[key]);
            });
            const statusCounts = _.countBy(unblessedResults, 'status');
            const unblessedCount = _.size(unblessedResults);
            output.error("\n", {sad: `Some failing tests (${unblessedCount} out of ${testCount}) remain:`});
